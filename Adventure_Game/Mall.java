    ...
    public Mall(Player player){
        ...
        if(player.getIntelligence() < 3){
            ...
            BigRat bigrat = new BigRat();
                bigrat.getDescription(); 
                combat.combat(player.getStrength(), player.getHealth(), bigrat);
        }
        else {
            System.out.println("As you escaped from the pack of rats, you run to the door for the stairways upward. There is a lock on the door that require a level of intelligence : 3. As your level of intelligence is " + player.getIntelligence() + " you can enter it. You quickly run up the stairs.");
        }
    }
